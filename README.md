...
MySQL 导入会自动读取上一批成功导入的 `cursor_end` 作为下一次的增量起点；首次导入没有历史游标时从空游标开始。需要补数或故障恢复时，可以显式传入 `--cursor 12345` 覆盖自动游标。 Use `latest_successful_cursor` in `status` output to verify the next automatic resume position. For manual `import-mysql`, failed rows return a non-zero exit code so schedulers and operators can catch partial imports.
