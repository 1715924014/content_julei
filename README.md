...
MySQL 导入会自动读取上一批成功导入的 `cursor_end` 作为下一次的增量起点；首次导入没有历史游标时从空游标开始。需要补数或故障恢复时，可以显式传入 `--cursor 12345` 覆盖自动游标。 Use `latest_successful_cursor` in `status` output to verify the next automatic resume position. Manual `import-csv` and `import-mysql` share the same failure signal: failed rows return a non-zero exit code so schedulers and operators can catch partial imports.
...
Daily JSON logs include `source_pending_after_batch`, `recommended_actions`, `recommended_commands`, `warnings`, `failure_stage`, `rows_failed`, and `limit_reached`. When `source_pending_after_batch` is greater than zero, run an extra import or temporarily raise `-Limit`; when `recommended_actions` is present, handle the listed repair, throughput, duration, doctor, or review-backlog action first. If a daily import fails before a batch summary is available, `daily_import_failed` marks the failure in `warnings`, `failure_stage=import_mysql_batch`, and `run_deployment_doctor` points operators to the copyable doctor command in `recommended_commands`; if another run is already holding the lock, `daily_lock_present` marks the overlap in `warnings` with `failure_stage=acquire_daily_lock`. `recommended_commands` contains copyable follow-up commands for common repair and review steps.
...
```powershell
python -m src.suggestion_pipeline export-db-results --db output_run_check/analysis.db --output-dir output_run_check/reports
```
