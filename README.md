...
Daily JSON logs include `source_pending_after_batch`, `recommended_actions`, `recommended_commands`, `warnings`, `rows_failed`, and `limit_reached`. When `source_pending_after_batch` is greater than zero, run an extra import or temporarily raise `-Limit`; when `recommended_actions` is present, handle the listed repair, throughput, duration, doctor, or review-backlog action first. If a daily import fails before a batch summary is available, `daily_import_failed` marks the failure in `warnings` and `run_deployment_doctor` points operators to the copyable doctor command in `recommended_commands`; if another run is already holding the lock, `daily_lock_present` marks the overlap in `warnings`. `recommended_commands` contains copyable follow-up commands for common repair and review steps.
