...
Daily job status and exit codes: `status=success` returns `0`; `status=partial` returns `1` when imported rows contain failures; `status=failed` returns `1` when the job aborts before completing. The JSON log is written under `logs/daily-mysql-*.json`. The scheduled script keeps daily logs for `LogRetentionDays` days by default, and `-LogRetentionDays 0` disables log cleanup. If log cleanup failed, the script writes a warning but still exits with the daily import result code.
