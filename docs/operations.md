...
```powershell
powershell.exe -ExecutionPolicy Bypass -File scripts/run_daily_mysql.ps1 -ProjectRoot D:\PyWorkspace\content_fenlei -ConfigPath config\mysql.prod.json -DbPath data\analysis.db -LogDir logs -BackupRoot backups -RecommendationOutputDir data -Limit 10000 -MaxDurationSeconds 3600 -MinThroughputRowsPerSecond 2
```
...
python -m src.suggestion_pipeline run-daily-mysql --config config\mysql.prod.json --db data\analysis.db --log-dir logs --recommendation-output-dir data --limit 10000 --max-duration-seconds 3600 --min-throughput-rows-per-second 2
```

Daily job status and exit codes: `status=success` returns `0`; `status=partial` returns `1` when imported rows contain failures; `status=failed` returns `1` when the job aborts before completing. The JSON log is written under `logs/daily-mysql-*.json`. The scheduled script keeps daily logs for `LogRetentionDays` days by default; the value must be zero or positive, and `-LogRetentionDays 0` disables log cleanup. If log cleanup failed, the script writes a warning but still exits with the daily import result code. Manual `import-csv` and `import-mysql` follow the same failure signal: failed rows return a non-zero exit code even when the batch record is written for later repair.

`--limit` must be a positive integer. Use the default daily value as the normal batch cap, and increase it temporarily only when `limit_reached` shows backlog. The scheduled script rejects invalid parameters with `Limit must be a positive integer`, `MaxDurationSeconds must be zero or positive`, or `MinThroughputRowsPerSecond must be zero or positive` before starting the daily job. Monitoring thresholds such as `--max-duration-seconds` and `--min-throughput-rows-per-second` must be positive finite numbers when enabled; use `0` for the script parameters to disable those optional checks.

Daily job logs include the post-import `health`, `recommended_actions`, `recommended_commands`, `pending_review_tasks`, `latest_successful_cursor`, `latest_batch_limit_reached`, `latest_batch_duration_seconds`, `latest_batch_rows_per_second`, `source_pending_after_batch`, and `source_pending_error_summary` fields when the status summary can be read. Use `-RecommendationOutputDir` in the scheduled script, or `--recommendation-output-dir` in `run-daily-mysql`, when recommended export commands should write repair CSVs outside the default `data` directory. When `source_pending_after_batch` is greater than zero, the log adds `source_backlog_remaining` to `warnings` and `run_additional_import_or_increase_limit` to `recommended_actions`. When the source pending count cannot be read, the log adds `source_pending_count_unavailable` to `warnings` and `inspect_source_pending_count` to `recommended_actions`; recommended_commands includes a doctor command for inspect_source_pending_count so operators can re-check config, password environment, and the analysis database before investigating source MySQL access. When the daily import fails before a batch summary is available, the log adds `daily_import_failed` to `warnings`, `failure_stage=import_mysql_batch`, plus `run_deployment_doctor` and a doctor command so operators can validate config, password environment, field mapping, and database access first. If summary capture fails, the import result is preserved and `health_summary_error` plus `health_summary_error_type` are written for troubleshooting.

A `daily-mysql.lock` file in the log directory prevents overlapping scheduled runs. If a new run sees this lock, it writes `daily_lock_present` to `warnings`, sets `failure_stage=acquire_daily_lock`, adds a status re-check command, and exits with code `1` without importing. Locks older than 6 hours are treated as stale and replaced automatically; the job log records `stale_lock_replaced=true`. Only remove a fresh lock manually after confirming no daily import process is still running.
...
- `recommended_actions`: machine-readable next steps derived from `health.reasons`, such as `inspect_running_import_or_lock`, `export_import_failures_and_repair_rows`, `run_additional_import_or_increase_limit`, `review_runtime_capacity`, `optimize_import_throughput`, and `review_pending_cluster_tasks`.
- `recommended_commands`: copyable commands derived from `recommended_actions`, such as `export-import-failures --latest` for failed rows or `export-review-tasks` for pending review work. recommended_commands includes a status command for inspect_running_import_or_lock so operators can re-check whether the latest import is still running before removing a lock or restarting the task. recommended_commands includes threshold status commands for review_runtime_capacity and optimize_import_throughput so operators can re-run the same duration or throughput check after capacity changes.
...
2. 打开最新的 `logs/daily-mysql-*.json`，查看 `status` 和 `error`。
   The log also records `duration_seconds`, `cursor_start`, `cursor_end`, `source_pending_after_batch`, `source_pending_error_summary`, `rows_read`, `rows_failed`, `limit_reached`, `warnings`, `daily_import_failed`, `daily_lock_present`, `lock_path`, `lock_started_at`, `stale_lock_started_at`, `error_type`, `failure_stage`, and `error_summary` for runtime, cursor progress, backlog detection, lock troubleshooting, and failure triage. Use `status --max-duration-seconds` to alert when recent completed batches are slower than expected.
