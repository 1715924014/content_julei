...
class DeploymentScriptTests(unittest.TestCase):
    def test_windows_daily_mysql_script_invokes_daily_job_with_strict_failure_handling(self):
        ...
        self.assertIn("$Limit -le 0", content)
        self.assertIn("Limit must be a positive integer", content)
        self.assertIn("$MaxDurationSeconds -lt 0", content)
        self.assertIn("MaxDurationSeconds must be zero or positive", content)
        self.assertIn("$MinThroughputRowsPerSecond -lt 0", content)
        self.assertIn("MinThroughputRowsPerSecond must be zero or positive", content)
