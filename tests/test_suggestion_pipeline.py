...
class SuggestionPipelineTests(unittest.TestCase):
    ...
    def test_status_rejects_non_finite_min_throughput(self):
        with patch("src.suggestion_pipeline.connect_analysis_db") as connect_db:
            from src.suggestion_pipeline import main
    ...
    def test_run_daily_mysql_passes_max_duration_threshold(self):
        ...
        self.assertEqual(exit_code, 0)
        self.assertEqual(run_job.call_args.kwargs["max_duration_seconds"], 30)

    def test_run_daily_mysql_passes_min_throughput_threshold(self):
        with patch("src.suggestion_pipeline.run_daily_mysql_job", return_value=0) as run_job:
            from src.suggestion_pipeline import main
        ...
        self.assertEqual(exit_code, 0)
        self.assertEqual(run_job.call_args.kwargs["min_throughput_rows_per_second"], 2.5)
    ...
    def test_status_recommended_commands_use_requested_db_path(self):
        with tempfile.TemporaryDirectory() as directory:
    ...
    def test_status_min_throughput_marks_slow_batch_unhealthy(self):
        with tempfile.TemporaryDirectory() as directory:
            db_path = Path(directory) / "analysis.db"
            with closing(connect_analysis_db(db_path)) as connection:
                storage = Storage(connection)
